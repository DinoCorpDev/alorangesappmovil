        ...
        <div class="card-body">
            <table class="table aiz-table mb-0">
                <thead>
                    <tr>
                        <th>#</th>
                        <th class="col-lg-3" data-breakpoints="lg">{{ translate('Service & rating') }}</th>
                            ...
                            <tr>
                                ...
                                <td>
                                    <span class="d-flex align-items-center">
                                        <img src="{{ uploaded_asset($review->service->thumbnail_img) }}"
                                            class="size-80px mr-2"
                                            onerror="this.onerror=null;this.src='{{ static_asset('/assets/img/placeholder.jpg') }}';">
                                        <span
                                            class="flex-grow-1 minw-0 text-truncate-2">{{ $review->service->name }}</span>
                                    </span>

                                </td>
                                <td class="lh-1-8">
                                    <span class="d-block">{{ translate('Name') . ': ' . $review->user->name }}</span>
                                    <span class="d-block">{{ translate('Email') . ': ' . $review->user->email }}</span>
                                    <span class="d-block">{{ translate('Phone') . ': ' . $review->user->phone }}</span>
                                    <span class="d-block">
